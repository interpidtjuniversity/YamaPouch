...
func configPortMapping(ep *Endpoint, cinfo *container.ContainerInfo) error {
	...
	for _, pm := range ep.PortMapping {
		...
		iptablesCmd := fmt.Sprintf("-t nat -A PREROUTING -d %s -p tcp --dport %s -j DNAT --to-destination %s:%s",
			localIp, portMapping[0], ep.IPAddress.String(), portMapping[1])
		exec.Command("iptables", strings.Split(iptablesCmd, " ")...).Run()

		iptablesCmd = fmt.Sprintf("-t nat -A POSTROUTING -d %s -p tcp --dport %s -j SNAT --to %s",
			ep.IPAddress.String(), portMapping[1], localIp)
		exec.Command("iptables", strings.Split(iptablesCmd, " ")...).Run()

		iptablesCmd = fmt.Sprintf("-t nat -A OUTPUT -p tcp -d 127.0.0.1 --dport %s -j DNAT --to %s:%s",
			portMapping[0], ep.IPAddress.String(), portMapping[1])
		exec.Command("iptables", strings.Split(iptablesCmd, " ")...).Run()

		iptablesCmd = fmt.Sprintf("-t nat -A OUTPUT -p tcp -d %s --dport %s -j DNAT --to %s:%s",
			localIp, portMapping[0], ep.IPAddress.String(), portMapping[1])
...
func debug(jsonStr string) {
	fileName := "/root/debug/debug.json"
